  ...
  onSubmit(): void {
    ...
    if (this.isEditMode && this.newsId) {
      this.newsService.updateNews(this.newsId, formData).subscribe(
        (response: any) => {
          AlertComponent.showSuccess('Noticia actualizada','Noticia actualizada exitosamente')
      ...
      this.newsService.createNews(formData).subscribe(
        (response: any) => {
          AlertComponent.showSuccess('Noticia creada','Noticia creada exitosamente')
          console.log('Noticia creada exitosamente', response);
          if (this.dialogRef) {
            this.notificationService.showNotification('Nueva publicación creada', {
